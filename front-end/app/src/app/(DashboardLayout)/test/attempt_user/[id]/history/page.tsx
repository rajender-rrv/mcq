...
type Status = "COMPLETED" | "Overdue" | "Pending" | "IN_PROGRESS";
    ...
    const fetchData = async () => {
      try {
        ...
        const formatted: Invoice[] = users.map((u: any, i: number) => ({
          id: u.id || i + 1,
          template_name: u.template_name || "N/A",
          to: u.email || "N/A",
          cost: Number(u.cost) || 0,
          status: ["COMPLETED", "Overdue", "Pending", "IN_PROGRESS"].includes(u.status)
            ? u.status
            : "IN_PROGRESS",
  ...
  return (
    <>
      ...
      <Box className="grid grid-cols-12 gap-6 mb-4">
        {["All", "COMPLETED", "Overdue", "Pending", "IN_PROGRESS"].map((t, index) => (
          <Box
            key={t}

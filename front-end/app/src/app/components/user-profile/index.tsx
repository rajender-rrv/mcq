...
const getCookie = (name: string) => {
  ...
  const getProfileImage = () => {
    if (role === "admin") {
       return "/images/profile/user-5.jpg";
    }
    ...
    <>
      ...
      <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center gap-2">
                {socialLinks.map((item, index) => (
                  <Link
                    ...
                    href={item.href}
                    target="_blank"
                    className="flex h-11 w-11 items-center justify-center gap-2 rounded-full shadow-md hover:bg-gray-50 dark:hover:bg-white/[0.03]"
                  >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="space-y-6 rounded-xl shadow-xs bg-background md:p-6 p-4">
            ...
            <div className="flex justify-end">
              <Button onClick={() => { setModalType("personal"); setOpenModal(true); }}>
          ...
          <div className="space-y-6 rounded-xl shadow-xs bg-background md:p-6 p-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div><p className="text-xs text-gray-500">State</p><p>{address.state}</p></div>
              <div><p className="text-xs text-gray-500">PIN</p><p>{address.pin}</p></div>
              <div><p className="text-xs text-gray-500">ZIP</p><p>{address.zip}</p></div>
              <div><p className="text-xs text-gray-500">Tax No</p><p>{address.taxNo}</p></div>
            </div>

            <div className="flex justify-end">
              <Button onClick={() => { setModalType("address"); setOpenModal(true); }}>
                <Icon icon="ic:outline-edit" width="18" height="18" /> Edit
              </Button>
            </div>
          </div>
      ...
      <Dialog open={openModal} onOpenChange={setOpenModal}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {modalType === "personal" ? "Edit Personal Info" : "Edit Address"}
            </DialogTitle>
          </DialogHeader>
          ...
          {modalType === "personal" ? (
            <Input
              ...
              onChange={(e) =>
                setTempPersonal({ ...tempPersonal, firstName: e.target.value })
              }
            />
          ) : (
            <Input
